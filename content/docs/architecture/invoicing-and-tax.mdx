...
description: Comprehensive guide to the invoicing lifecycle, tax engine, and EU VAT OSS reporting.
---
...
Invoices enforce a strict immutability rule designed for accounting integrity. They transition through specific statuses:
...
2. **`issued`**: The invoice becomes a finalized accounting document. The invoice number is permanently assigned. From this point forward, the core contents cannot be mutated.
3. **Void / reversal workflow**: If an issued invoice must be negated, the platform performs a formal void / reversal workflow rather than mutating the invoice in place. The original invoice remains part of the accounting record.
...
Once an invoice reaches the `issued` state, standard mutations are blocked. However, minor corrections, such as typographical errors in addresses, missing metadata, or a corrected buyer Peppol routing address snapshot, can be requested through the invoice correction workflow.
...
Unlike the legacy `transaction.externalReference`, which holds the merchant-provided identifier, invoice numbering commonly requires gapless, sequentially incrementing numbers.
...
- If a draft invoice line omits `taxRateCode`, the Admin UI keeps the VAT field in automatic mode and shows the previewed backend result separately. Selecting a VAT rate code is an explicit manual override; the `Auto` action clears that override and returns the line to automatic rule resolution.
...
VAT OSS uses the same active tax records and line-level tax basis as the other reports. Goods and shipping lines resolve destination from shipping country with billing as fallback; digital service, tax, and discount lines resolve destination from billing country. Mixed goods and digital-service documents therefore keep separate line-level countries and rate codes, while document-level country remains summary metadata. EU cross-border B2C goods and services are OSS candidates; non-EU destination lines are treated as exports and stay out of VAT OSS.
