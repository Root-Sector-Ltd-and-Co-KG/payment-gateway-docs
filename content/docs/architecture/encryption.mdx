...
| ----------------------- | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **Purpose**             | Protects platform and tenant configuration data: admin users, organization profile fields, provider configuration, site labels, and saved organization/site SMTP or geolocation secrets. | Protects tenant runtime/customer data: clients, sessions, transactions, invoices, credit notes, and queued notification payloads. |
...
- **Algorithm:** AES-256-GCM with a random nonce per encryption; additional authenticated data (AAD) binds ciphertext to scope, organization id for organization scope, collection, field name, and DEK version.
...
Because the AAD includes the encryption context, encrypted field metadata is not portable. Code that creates a credit note from an invoice, or any similar document snapshot, must copy plaintext values and let the target document encrypt them under its own collection and field context. Copying ciphertext, IV, version, or search hash between contexts can make the target field undecryptable.
...
Saved SMTP, geolocation, provider, and site configuration secrets are protected with the **system** encryption context even when they belong to a specific organization. Queued notification payloads and customer-facing document data remain organization-scoped runtime data.
