...
The Admin Backend is a Go service using Gin, MongoDB, WebAuthn, JWT sessions, API-key authentication, OpenTelemetry middleware, structured logging, and the shared crypto libraries.

### Core Package Structure

Most business areas follow the same pattern under `internal/`:

- **Feature packages** such as `checkouts`, `invoices`, `transactions`, `providers`, `domains`, `tax`, `dataio`, and `system` contain handlers and business services for one domain.
- **`server/`** owns route registration, health checks, rate limiting, auth middleware, license middleware, request IDs, CORS, and Swagger exposure.
- **`database/`** owns MongoDB connection setup, collection constants, indexes, and migrations.
- **`server/middleware/`** provides JWT auth, API-key auth and scope checks, API-key audit logging, license enforcement, rate limiting, logging, and request timeouts.
- **`organizations/settings/*`** and **`system/settings/*`** separate tenant-level settings from global installation settings.
...
6. **Invoicing and documents**: Invoice issue, void, correction, credit note, recurring schedule, payment checkout, PDF regeneration, ZUGFeRD/PDF, XML, and Peppol XML routes are owned by the admin backend. Slow document generation is queued to the worker.
