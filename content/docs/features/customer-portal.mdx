...
3. **Session issuance**: After the link or OTP is accepted, the backend exchanges it for a signed portal session token (opaque bearer token), granting read-only access to their specific Organization-bound Client profile.
