---
title: Invoicing & Credit Notes
description: Managing the lifecycle of invoices, corrections, and refunds.
---

# Invoicing & Credit Notes

Invoices are the accounting backbone of your transactions. The Payment Gateway generates locked invoice PDFs and configured e-invoice XML formats for successful captured payments. Your organization remains responsible for validating document settings, numbering, tax treatment, and jurisdiction-specific legal requirements.

## Document Detail Views

In the Admin UI, invoice and credit note detail pages use the same party structure:

- **Seller**: The legal entity issuing the document.
- **Client**: The billed counterparty shown on the document snapshot.

This keeps invoices, credit notes, and transaction detail views consistent inside the admin control plane.

When the document is linked to a stored Client record, the Client email in the detail view links directly to that Client profile. This gives operators a quick path from a document to the broader relationship history without changing the legal document itself.

The document still preserves its own party snapshot. If the current Client profile changes later, the already-issued invoice or credit note continues to show the original document data that was valid at issuance time.

![Invoice detail showing seller and client snapshots, payment state badges, totals, document actions, line items, and billing details](/screenshots/app/invoice-detail.png)

## Invoice Statuses

Invoices exist in strict states to maintain accounting integrity:

1. **`draft`**: The invoice is currently being constructed. You can edit line items, change the billing address, or delete it entirely without leaving an accounting trace.
2. **`issued`**: Once the "Issue" button is clicked (or a transaction succeeds automatically), the invoice is locked. An official sequential `InvoiceNumber` is permanently assigned. **The line items and total amounts can never be changed again.**
3. **Payment state (computed)**: Whether an invoice is open, partially settled, settled, credited, refunded, or overpaid is derived from linked settlements and credit notes rather than stored as a separate mutable invoice status.
4. **Void / refund workflows**: If an issued invoice is critically wrong or must be reversed, the platform creates the appropriate reversal artifact and keeps the original invoice on file as part of the permanent accounting record.

## Draft Invoice Tax Calculation

When creating or editing a draft invoice manually, VAT can be either automatic or explicit on a per-line basis.

1. Leave the VAT field blank to let the backend calculate tax automatically.
2. Enter a VAT percentage only when you intend to override the automatic result for that line item.
3. The Admin Panel now previews the canonical backend tax result before save, including the resolved rate code when available.

The same tax engine is used for transactions, recurring invoices, API-key draft invoice creation, and the draft-invoice preview endpoint, so the behavior stays consistent across flows. For lifecycle, OSS evidence, and ledger concepts, see [Invoicing & Tax architecture](../architecture/invoicing-and-tax); for `POST /api/v1/invoices/preview-tax` and tax report routes, see [Admin API](../api-reference/admin-api).

When a draft is issued, the invoice stores the resulting tax snapshot and writes canonical tax records. Depending on the organization tax recognition mode, those records are recognized at invoice issue time (`accrual`) or deferred until settlement (`cash`). Credit notes write separate offsetting tax records; the original invoice tax snapshot remains unchanged.

## Payment Links

Issued invoices can have hosted payment links created manually from the invoice payment section or automatically during issue when **Settings > Invoicing > Auto-create payment link on issue** is enabled.

Manual invoice payment links default to the current outstanding amount, but operators can create a smaller partial-payment link. The backend caps the amount at the outstanding balance, enforces a `0.50` minimum, and rejects empty or fully settled invoices.

Payment-link expiry uses the same invoicing setting as automatic issue-time links. If no value is configured, the default is `336` hours (14 days); explicit expiry values can be between `1` and `3120` hours.

For reverse-charge, export, exempt, zero-rated, and manual VAT outcomes, invoice checkout creation keeps the invoice tax snapshot but avoids sending synthetic tax-rate codes to checkout tax-rate lookup. When a transaction settles a partial link, the invoice settlement logic allocates tax proportionally from the invoice.

## The Customer Portal

Rather than emailing large PDF attachments that may be blocked by spam filters, the gateway hosts a secure **Customer Portal**.

End-users navigate to your custom domain (e.g., `invoices.yourcompany.com`) and authenticate using a one-time secure link, with OTP confirmation available where configured. From there, they can independently:

- Download their invoice as a PDF.
- Download their invoice as a machine-readable native E-Invoice XML (ZUGFeRD/Factur-X CII, XRechnung, Peppol) if configured.

E-invoice settings control the formats and routing identifiers available for portal downloads and email workflows.

![E-invoicing settings page showing XRechnung, ZUGFeRD, Peppol, validation, Leitweg-ID, and seller identity fields](/screenshots/app/einvoicing-settings.png)

## Credit Notes (Refunds)

Because `issued` invoices are strictly immutable, you cannot simply edit the total amount to reflect a refund.

Instead, you must issue a **Credit Note**. A Credit Note is a separate legal document that mathematically counters the original invoice.

The original invoice always remains part of the accounting record. Refund and reversal outcomes are represented by linked settlement totals and credit notes, not by editing or replacing the invoice itself.

If an invoice is linked to payment records, the authoritative reversal workflow is **refund-first**.

1. Open the Invoice or the linked successful Transaction.
2. If exactly one refundable transaction is linked, click **Refund Payment** directly from the Invoice screen and complete the same refund form used in the Transaction screen.
3. If multiple refundable transactions are linked, choose the relevant Transaction from the Invoice payment section and process the refund there.
4. Choose to perform a full or partial refund.
5. The system communicates the refund request to the upstream Provider (e.g., Stripe), records the refund against the linked invoice settlement state, and generates the Credit Note document for the Client.

When an invoice has **no** linked payment refund path, the Invoice screen offers a **Create Credit Note** workflow for document-side full or partial reversals.

Credit notes therefore serve two purposes:

- They provide the reversal document used for accounting and tax records.
- They explain why the invoice payment state may move to partially refunded, refunded, or fully credited without changing the original invoice contents.

Tax reporting follows the same principle. Refunds, fund-impacting chargebacks, and credit notes are represented as negative adjustment records linked to the original sale or invoice; the original tax record remains part of the audit trail. Adjustment records preserve line-level tax country and VAT rate code where available, which keeps mixed physical and virtual sales aligned with their original reporting basis. If the source only has aggregate tax breakdown data, the adjustment still preserves untaxed residuals as their own line basis and caps over-allocated breakdowns to the recorded refund or chargeback total.

Refund-created credit notes inherit the original invoice's Seller and Client snapshot. That means the reversal document stays legally aligned with the invoice it offsets, even if the current Client profile has changed since the invoice was issued.

Chargebacks follow dispute outcome. Open or under-review chargebacks can reduce revenue reporting as payment-risk adjustments, but they do not issue a legal credit note yet. If the dispute is lost or accepted, the platform creates the chargeback credit note; if the dispute is won, active chargeback tax adjustments are voided and no credit note is issued.

> [!IMPORTANT]
> The platform intentionally does not offer a paid-invoice workflow that creates a stand-alone credit note while leaving the linked payment untouched. For paid invoices, the credit note should follow the refund workflow so cash movement, settlement state, and accounting documents remain aligned.

## Numbering Configuration

Organizations control invoice and credit note numbering from the invoicing settings page.

Each organization configures two independent document layouts: one for invoices and one for credit notes.

Each layout can define:

- a dedicated prefix
- optional middle text between the visible period and the sequence
- an optional suffix
- the first sequence number to allocate
- the zero-padding width of the numeric sequence
- whether the visible period uses no date, the calendar year, or the fiscal year
- whether the sequence resets never, every calendar year, or every fiscal year
- how much of the issue date appears in the number
- how date parts are separated when month or day are shown

If a layout uses `No date`, the sequence is appended directly after the prefix unless you provide explicit middle text. This avoids accidental doubled separators for prefixes that already end in punctuation such as `-`.

Typical layouts include:

- `No date` → example: `INV-0001`
- `Year` → example: `INV-2026-0001`
- `Year + Month` → example: `INV-2026-03-0001`
- `Year + Month + Day` → example: `INV-2026-03-27-0001`
- `Fiscal year with middle text` → example: `INV2027AR0001`
- `Fiscal year with suffix` → example: `CN-2026-0001-REV`

When the separator is set to `None`, date parts are compacted:

- `INV-20260327-0001`

When fiscal numbering is enabled, the backend derives the year label from the actual issue date and the configured fiscal-year start month. That same fiscal definition also drives the optional fiscal-year reset policy.

The sequence itself remains gapless and organization-scoped. Changing the format affects newly allocated numbers only. Existing invoices and credit notes keep their assigned document numbers permanently.

## Document Template Customization

Organizations can create custom PDF document templates for invoices, pro forma documents, credit notes, and receipts. The template editor is shared across these document types, but each type has its own default template and type-specific labels.

Typical workflow:

1. Open `Settings > Templates`.
2. Select the document type: `Invoice`, `Pro Forma`, `Credit Note`, or `Receipt`.
3. Create a new document template from a preset, import a template package, or duplicate a system default.
4. Adjust branding, drag or move section order, page regions, optional section visibility, footer content, watermark behavior, and supported variables in Designer mode.
5. Generate a PDF proof to verify the actual A4/Letter output.
6. Publish and set the template as the default for that document type, or select an invoice/pro forma template when creating or editing a draft invoice.

You can also assign a specific PDF template in invoicing settings so newly generated invoices use that template automatically. Sites may override the inherited organization template when a storefront needs different branding.

This means custom document templates are supported as:

- organization defaults for invoice generation
- per-site branding overrides
- per-invoice selection during draft invoice creation or editing
- separate defaults for pro forma, credit-note, and receipt layouts

For issued documents, `standard` and `corrective` invoice business types resolve to the `invoice` document template type. Draft pro forma documents resolve to the `proforma` template type. When a site-level PDF template override is configured, the backend treats that setting as a concrete document template ID.

The template management screen is the place to preview default layouts, duplicate a template before editing, and assign the resulting template at the organization or site level.

### Template Source Modes

Designer templates do not expose generated HTML as their editable source. Their canonical source is the semantic `builderDefinition`, which contains the document page setup, section outline, visibility rules, layout settings, and channel-specific blocks. The **Source & Export** workspace shows this Designer JSON and the backend-generated render artifact separately so templates can be exported and imported without confusing generated output with editable source.

When full control over HTML or text is required, a user can convert a template to **Advanced Source**. Advanced Source uses authored HTML for documents and email, markdown/text for Slack, and text for SMS. After conversion, Designer controls are disabled for that template because arbitrary source cannot be reliably parsed back into semantic builder settings.

Template export packages use the schema `payment-gateway.template.export.v1`. Imports show a package review step, then create a new draft and never overwrite an existing default or published template. Issued documents continue to use their locked published revision artifacts.

### Template Variables

Template variables use Go template syntax such as `{{.InvoiceNumber}}`. The variable panel groups available fields so Designer content blocks or Advanced Source bodies can be edited without guessing the data shape:

- **Document**: numbers, issue date, currency, totals, and type-specific fields such as `InvoiceNumber`, `ProFormaNumber`, `ReceiptNumber`, `CreditNoteNumber`, `OriginalInvoiceNumber`, and `Reason`. Due date, validity date, draft state, and payment-link fields are shown only for document types whose render data supports them.
- **Seller**: seller identity, contact data, address, VAT ID, national tax ID, commercial register, managing director, and website.
- **Client**: billed client identity, email, phone, VAT ID, client identifiers, billing address, and shipping address.
- **Payment**: payment terms, bank details, hosted payment link, and payment-link QR code.
- **Line Items**: fields used inside `{{range .LineItems}}...{{end}}`, including description, quantity, net unit price, `VATRate`, `VATAmount`, net amount, and gross line total.
- **VAT Breakdown**: fields used inside `{{range .VATBreakdown}}...{{end}}`, including `DisplayLabel`, `Rate`, `Amount`, `VATBase`, `CountryCode`, `CountryName`, and `CountryLabel`.
- **Document Notes**, **Footer**, and **Custom**: notes, small-business exemption text, logo/footer fields, and user-defined custom variables.

The VAT breakdown variables are country-aware for EU/VAT reporting. If the tax rate code includes a recognizable country, `DisplayLabel` renders labels such as `17% VAT (LU/Luxembourg)`, while `CountryCode`, `CountryName`, and `CountryLabel` are available separately for custom tables. If no country can be derived, the label falls back to the VAT rate only.

Shipping address variables are exposed separately from billing address variables under `Client.ShippingAddress.*`. The default templates render a **Shipping Address** block only when at least one shipping-address field is present. Physical goods and shipping lines can use the shipping destination for tax calculation, but the template does not invent a shipping address when the document snapshot does not contain one.

Custom variables defined on a template can be supplied when a draft invoice is created or when a transaction-based invoice is generated. Use `templateVariables` for invoice templates and `proFormaVariables` for draft/pro forma templates. Values are rendered as top-level template fields, such as `{{.CustomDeliveryNote}}`, and are also available through `{{index .CustomVariables "CustomDeliveryNote"}}`.

### Default Layout Semantics

The system defaults for invoice, pro forma, credit note, and receipt templates share a compact business-document layout:

- A seller sender line and billed-client address appear on the left; document metadata, logo, number, and date appear on the right.
- Billing address is the primary recipient address. Shipping address is shown as a secondary block only when present.
- Line items show net unit price, VAT rate, VAT amount, and gross line total.
- The lower summary shows subtotal first, then country-aware VAT breakdown rows, then the total amount.
- VAT breakdown rows prefer the country-aware `DisplayLabel`.
- Invoices can include payment terms, required invoice notes, small-business exemption text, and a hosted payment link with QR code.
- Pro forma templates label the due-date field as **Valid Until** and display a pro forma watermark for draft previews.
- Credit note templates show the credit-note number, original invoice number, reason, and negative amount semantics while preserving the original invoice snapshot basis.
- Receipt templates use receipt-specific labels and receipt confirmation text for payment confirmations.

The document renderer reads the v2 builder `sectionOrder` first. Existing templates without a section outline are normalized into the default order before rendering. PDF proofs and runtime document generation use the same backend render path so the in-app proof reflects the issued document layout more closely than a browser-only HTML preview.
