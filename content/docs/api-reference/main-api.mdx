...
The endpoint uses organization tax settings, effective-dated tax rates, tax rules, gross/net price mode, rounding, shipping destination overrides for physical/shipping lines, and VIES-backed buyer type only when the effective checkout VAT validation mode allows VIES.
...
Returns the requested e-invoice XML when that format is enabled for both organization eInvoice generation and portal downloads. Supported `format` values are `xrechnung`, `peppol`, and `zugferd`; `zugferd` returns standalone Factur-X/ZUGFeRD CII XML. Standard invoice PDFs are downloaded through `/api/v1/portal/documents/:id/pdf`.
