...
The Admin Backend is the primary integration surface for server-side automation. Public integrations usually call it on the configured API domain. Proxy configs and backend code may also refer to the same handlers under `/api/v1`. All endpoints require an **API Key** or **JWT** in the `Authorization: Bearer` header.
...
| ------- | ---------------------------------- | ----------------- | -------------------------------- |
| `POST`  | `/api/v1/checkouts/:siteId/create` | `checkout:create` | Create a checkout URL for a site |
| `GET`   | `/api/v1/checkouts`                | `checkout:read`   | List all checkouts               |
| `GET`   | `/api/v1/checkouts/:id`            | `checkout:read`   | Get a checkout by ID             |
| `PATCH` | `/api/v1/checkouts/:id`            | `checkout:update` | Update checkout metadata         |

### Create Checkout

```http
POST /v1/checkouts/{siteId}/create
Authorization: Bearer sk_live_keyid.secret
Content-Type: application/json
```

> [!NOTE]
> The example above uses the public `/v1` API surface. If you are reading backend router code or reverse proxy rules, you may see the same route represented internally as `/api/v1/checkouts/:siteId/create`.
...
```json
{
  ...
  "checkoutUrl": "https://pay.yourcompany.com/checkout/abc123",
