...
The due-schedule processor generates invoices for schedules whose `nextRunDate` is due. Generated invoices remain drafts unless the schedule has `autoFinalizeInvoices` enabled. Automatic email sending only happens when schedule and organization email settings allow it; recurring processing does not automatically capture payment-provider funds.
