...
The e-invoice download endpoints enforce **Settings > Invoicing > eInvoice Generation** before proxying to the worker. Requests return `403 EINVOICE_DISABLED` when eInvoice generation is disabled and `403 EINVOICE_FORMAT_NOT_ALLOWED` when the requested format is not listed in `settings.invoicing.eInvoice.allowedFormats`.
