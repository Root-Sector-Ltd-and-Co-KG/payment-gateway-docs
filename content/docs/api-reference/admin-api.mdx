...
| `POST` | `/api/v1/invoices/recurring/:id/cancel`   | `invoice:execute` | Cancel schedule or request cancellation for future cycles              |
...
| `POST` | `/api/v1/system/recurring/process`        | `system:execute`  | Process all currently due schedules (global/system automation surface) |
...
The due-schedule processor generates invoices for schedules whose `nextRunDate` is due. Generated invoices remain drafts until an operator issues them unless the schedule has `autoFinalizeInvoices` enabled. `autoSendInvoices` is valid only when `autoFinalizeInvoices` is also enabled; recurring auto-send is the schedule-level email trigger after automatic issue. Recurring processing does not automatically capture payment-provider funds.
