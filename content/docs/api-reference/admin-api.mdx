...
**Shorter public path (default reverse proxy):** same host with `/v1/...`. The default reverse proxy configuration shipped with the deployment package rewrites `/v1/*` to `/api/v1/*` on `{$ADMIN_BACKEND_DOMAIN}` before proxying to the Admin Backend.
...
> [!IMPORTANT]
> Provide at least one of `externalReference` or `invoiceNumber`. The API accepts both, but a reference is required for reconciliation.

> [!NOTE]
> `{transactionId}` in `returnUrl`/`cancelUrl` is a literal template placeholder — the gateway substitutes the actual transaction ID at redirect time.
...
| `GET`    | `/api/v1/transactions/bulk-recalculate-tax`         | `transaction:read`    | Get latest tax-record rebuild job          |
| `POST`   | `/api/v1/transactions/bulk-recalculate-tax`         | `transaction:execute` | Start tax-record rebuild                   |
| `GET`    | `/api/v1/transactions/bulk-recalculate-tax/:jobId`  | `transaction:read`    | Poll tax-record rebuild job status         |
...
| `GET`    | `/api/v1/invoices/:id/xml`                         | `invoice:execute` | Download XRechnung XML                                     |
| `GET`    | `/api/v1/invoices/:id/xml/peppol`                  | `invoice:execute` | Download Peppol BIS 3.0 XML                                |
...
The e-invoice download endpoints enforce **Settings > Invoicing > eInvoice Generation** before proxying to the worker. Requests return `403 EINVOICE_DISABLED` when eInvoice generation is disabled, `403 EINVOICE_FORMAT_NOT_ALLOWED` when the requested format is not listed in `settings.invoicing.eInvoice.allowedFormats`, `409 INVOICE_NOT_ISSUED` when a draft invoice requests XML or hybrid eInvoice output, and `422 EINVOICE_READINESS_FAILED` when the issued invoice snapshot or organization settings are missing required readiness data such as IBAN or Peppol routing addresses. Draft invoices only support pro forma PDF download; the final invoice number is assigned during issue.

Issued invoices keep their party snapshot. To copy or correct a buyer Peppol routing address for an already issued invoice, use the normal correction endpoint. The correction updates the invoice snapshot only; it does not silently mutate the linked client record.

```json
{
  "reason": "Copied buyer Peppol routing address from linked client for eInvoice generation",
  "changes": {
    "client": {
      "peppolRoutingAddress": {
        "schemeId": "9930",
        "value": "DE987654321"
      }
    }
...
`POST /api/v1/invoices/:id/payment-checkout` creates a hosted payment link for an issued invoice.
...
- `amount` is in minor units. If omitted, the backend uses the invoice's payable outstanding amount.
- Partial payment links are allowed from `50` minor units up to the outstanding amount.
- Empty invoices, fully settled invoices, zero-outstanding invoices, and amounts below the minimum return `400`.
- If `expireInHours` is omitted, the backend uses the effective invoicing setting `autoPaymentLinkExpiresInHours`, falling back to `336` hours. Explicit values must be between `1` and `3120`.
- Reverse-charge, export, exempt, zero-rated, and manual synthetic invoice tax codes are not sent as checkout tax-rate codes; settlement tax allocation still follows the invoice tax snapshot.
...
| `GET`  | `/api/v1/invoices/recurring/:id`          | `invoice:read`    | Get schedule detail                                                    |
| `PUT`  | `/api/v1/invoices/recurring/:id`          | `invoice:update`  | Update schedule fields and template line items                         |
...
| `POST` | `/api/v1/invoices/recurring/:id/cancel`   | `invoice:execute` | Cancel schedule or schedule future-cycle cancellation                  |
...
| `POST` | `/api/v1/system/recurring/process`        | `system:execute`  | Process all currently due schedules and due scheduled cancellations    |
| `GET`  | `/api/v1/system/recurring/status`         | `system:read`     | Read recurring processor status and metrics                            |

Recurring schedule payloads and responses use `nextRunDate` for the next scheduled processing date. Use `startDate` when creating a schedule; the backend materializes `nextRunDate` from the start date and interval. Do not use the older `nextRunAt` name in integrations.

Updating `startDate` also moves `nextRunDate` before any invoices have been generated. Once a schedule has generated invoices, `startDate` can only move future billing after the current `nextRunDate`; moving it onto the current cycle or an earlier date returns a validation error.

The cancel endpoint accepts an optional JSON body: `{ "reason": "..." }`. The backend trims the reason, caps it at 500 characters, stores cancellation audit metadata (`cancellationRequestedAt`, `cancelledBy`, and `cancellationReason`), and keeps empty-body cancellation requests backward compatible.

The due-schedule processor generates invoices for active schedules whose `nextRunDate` is due. It also finalizes `cancel_requested` schedules whose `effectiveFromCycle` is due, with `nextRunDate` fallback for older records. Generated invoices remain drafts until an operator issues them unless the schedule has `autoFinalizeInvoices` enabled. `autoSendInvoices` is valid only when `autoFinalizeInvoices` is also enabled; recurring auto-send is the schedule-level email trigger after automatic issue. Recurring processing does not automatically capture payment-provider funds.

The Admin Panel labels the manual processor action **Run Due Schedules** because it does not force future scheduled cancellations early. The response keeps `processed`, `completed`, `failed`, and `skipped`, and also includes `cancellationsFinalized` so cancellation-only runs are visible in UI summaries and processor metrics.
...
Client tax data may include a buyer Peppol routing address only for customers that receive Peppol BIS 3.0 XML. The value is encrypted like other client compliance fields. Use `peppolRoutingAddress`, not older generic endpoint names.

```json
{
  "taxInfo": {
    "taxId": { "plaintext": "DE987654321" },
    "taxIdType": "vat",
    "taxCountry": "DE",
    "peppolRoutingAddress": {
      "schemeId": "9930",
      "value": { "plaintext": "DE987654321" }
    }
  }
}
```

Do not assume the VAT ID is the Peppol address. Use a VAT-number scheme such as `9930` only when the buyer confirms that exact Peppol address. Use `0088` only for a supplied GS1 GLN. Use `0204` for a German Leitweg-ID only when the German public-sector recipient confirms it as their Peppol receiver address.
...
```json
{
  ...
  "bankIBAN": { "plaintext": "DE89370400440532013000" },
  "eInvoice": {
    "enabled": true,
    "allowedFormats": ["xrechnung", "peppol", "zugferd"],
    "strictValidation": true,
    "sellerPeppolRoutingAddress": {
      "schemeId": "9930",
      "value": { "plaintext": "DE123456789" }
    }
  },
  "invoiceNumberLayout": {
    "prefix": "INV-",
...
| `POST`   | `/api/v1/tax-rules/populate-defaults` | `tax_rule:create`  | Create missing default tax rules; returns an empty `created` array when already up to date |
...
| `POST`   | `/api/v1/tax-rates/replace-defaults`  | `tax_rate:execute` | Sync managed official default rates without deleting custom rates |
...
Immutable **tax records** back admin reporting drill-downs and traceability. List filters include date range, `sourceKind`, `recognitionMode` (`accrual` / `cash`), pagination, and links to transactions or invoices.
...
| `tax_rate:execute`     | Sync managed official default tax rates (`replace-defaults`)                                     |
...
| `system:read`          | Read system-level operational status endpoints                                                  |
| `system:execute`       | Execute system-level operational actions such as recurring due-schedule processing              |
...
- **Recurring schedule management**: `invoice:read`, `invoice:create`, `invoice:update`, `invoice:execute`
- **Recurring processor operations**: `system:read`, `system:execute` (system API keys or global administrator sessions)
