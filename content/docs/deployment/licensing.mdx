...
Use **System > Licensing** as the source of truth for local runtime state, activation, **Sync License Now**, support window, and last synchronization time. Only global administrators can open this page. Organization administrators can still use the non-portal recovery settings pages while runtime access is blocked.
...
3. Use **System > Licensing > Sync License Now** after any customer portal or operator portal change that should affect the self-hosted host immediately.
...
1. In the customer portal or operator workspace, confirm whether the remote status is manual `BLOCKED`, `SUSPENDED`, `REVOKED`, or a version/support policy block.
...
2. Confirm the intended license is usable in the customer portal or operator workspace.
