...
5. **Initial Activation (`NOT_ACTIVATED`)**: Configure your license key under **Settings > Licensing** in the Admin Panel and complete the activation process. A successful activation also updates the customer dashboard at `payment-gateway.app` so the license no longer appears as `Unassigned`.
