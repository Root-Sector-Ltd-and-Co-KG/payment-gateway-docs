...
```bash
node payment-gateway-local-dev/local-dev.mjs db audit encryption
```

The audit checks for copied encrypted snapshot payloads and scans local backend logs for system-scope and organization-scope authentication failures.
