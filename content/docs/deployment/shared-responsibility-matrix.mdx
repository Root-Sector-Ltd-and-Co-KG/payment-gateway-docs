...
Use this matrix during onboarding, audits, and security reviews.

## Responsibility Overview

| Control Area | Application Provides | Operator Must Provide |
| --- | --- | --- |
| Payment data boundary | Provider-tokenized card flow patterns; no raw PAN/CVC storage by gateway services | Provider account configuration, checkout-domain security, integration review, and awareness that custom checkout extensions can change PCI DSS scope |
...
| Release security artifacts | Release metadata, component digests, SBOMs, vulnerability reports, and vulnerability policy evidence where published | Review artifacts before rollout, verify checksums/provenance where available, retain update evidence, and assess accepted waivers |
