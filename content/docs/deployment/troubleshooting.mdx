...
> [!NOTE]
> Screenshot placeholder: **Domains / Site custom domain detail** showing verification status, DNS target, and TLS readiness.
...
> [!NOTE]
> Screenshot placeholder: **Invoice detail** showing PDF generation state, regenerate action, and download buttons.
...
1. Run the local ledger audit against the affected environment:
   ```bash
   node payment-gateway-local-dev/local-dev.mjs db audit tax
   ```
2. Confirm there are no active `tax_records` with missing line-level tax country.
3. Confirm issued invoices and issued credit notes have active canonical tax records.
4. Confirm issued document `taxRecordIds` point at active records for the same source document, not superseded records or transaction-sourced records.
...
1. Run the encryption audit:
   ```bash
   node payment-gateway-local-dev/local-dev.mjs db audit encryption
   ```
2. Check whether failures are organization-scope or system-scope in the audit output.
...
5. After repair, rerun the tax audit and Rebuild Tax Records.
