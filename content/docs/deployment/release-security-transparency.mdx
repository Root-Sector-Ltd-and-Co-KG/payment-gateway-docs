...
Download the checksum manifest, signature bundle, and every file named by the
checksum manifest for the product version you are applying:
...
8. Verify `SHA256SUMS` with `sha256sum -c SHA256SUMS`, verify
   `SHA256SUMS.sigstore.json` with `cosign verify-blob`, and retain
