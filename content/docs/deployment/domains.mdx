...
description: Reference subdomains for hosted and self-hosted Payment Gateway deployments.
---

# Hostnames & DNS conventions

Use this page as the canonical list of application hostnames and how they relate to deploy environment variables. Generic examples elsewhere (`yourcompany.com`, `example.com`) illustrate the same roles under your own DNS zone.

---

## Reference: `*.payment-gateway.app` Hosted Stack

The hosted platform uses a fixed subdomain layout. Each hostname maps to the corresponding variable in Docker Compose / Podman deploy:
...
| `https://api.payment-gateway.app`       | `ADMIN_BACKEND_DOMAIN`  | Admin API - Organization API keys, `POST /api/v1/checkouts/{siteId}/create`, management REST                                         |
...
Merchant custom domains for buyer-facing checkout are additional hostnames, verified in the Admin Panel and typically served via on-demand TLS when the merchant hostname reaches the gateway origin directly. See [Caddy reverse proxy](./caddy).

> [!IMPORTANT]
> Custom domains are frequently part of payment-provider onboarding, domain verification, and risk review, not just branding. Providers may require your checkout pages, callback URLs, and webhook endpoints to remain on domains you own and control.
...
Cloudflare reference: [Create custom hostnames](https://developers.cloudflare.com/cloudflare-for-platforms/cloudflare-for-saas/domain-support/create-custom-hostnames/).
...
Variable names and a worked `example.com` table: [Self-Hosted Installation](../getting-started/installation). Routing and the optional `/v1` to `/api/v1` rewrite: [Caddy reverse proxy](./caddy). Integration order (webhooks -> API key -> checkout -> IPN): [Server integration checklist](../getting-started/integration-checklist).
