...
**Merchant custom domains** (buyer-facing checkout on the merchant’s own DNS) are **additional** hostnames, verified in the Admin Panel and typically served via on-demand TLS when the merchant hostname reaches the gateway origin directly — see [Caddy reverse proxy](./caddy).
