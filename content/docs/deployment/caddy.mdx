...
The deployment package ships a customized Caddy reverse proxy with MongoDB-backed certificate storage. TLS certificates and ACME challenge data are stored in your existing MongoDB instance, eliminating the need for a shared filesystem between Caddy instances.

**Canonical configuration:** default routing, rewrites, TLS storage, and security headers are managed by the reverse proxy configuration shipped with `payment-gateway-deploy`. This page summarizes the customer-facing behavior.
...
Placeholders such as `{$ADMIN_BACKEND_DOMAIN}` are expanded from the environment when the container starts (see your deploy stack). **Specific server blocks must appear before the catch-all `:443` block**; the shipped configuration already uses the required order.
...
This is the relevant behavior from the shipped reverse proxy configuration; health checks, default headers, and optional custom backend options are managed in the deployment package.

> [!NOTE]
> The API-bearing `reverse_proxy` blocks set `header_up X-Real-IP {http.request.client_ip}`.
> Caddy resolves `{http.request.client_ip}` using the global `servers > trusted_proxies`
> list, then **overwrites** any client-supplied `X-Real-IP` with that authoritative value.
> The Go backends only honour forwarding headers when the connecting peer is inside
> `MPG_SECURITY_TRUSTED_PROXIES` (which defaults to the internal container networks), so
> the real client IP used for rate limiting and session IP binding cannot be spoofed.
> This works **with or without Cloudflare** in front — if you run an external load
> balancer instead, add its CIDR(s) to both Caddy's `trusted_proxies` and the backends'
> `MPG_SECURITY_TRUSTED_PROXIES`. See [Environment Variables](./environment-variables#security--network-trust).
