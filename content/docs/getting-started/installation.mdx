...
Deploy the Payment Gateway stack on your own infrastructure with Docker Compose. This guide covers domains, image access, first startup, license activation, and updates.
...
- Deployment access credentials from your license dashboard
- Inbound ports `80` and `443` open
- Outbound HTTPS to the vendor endpoints shown in your deployment access snippet
...
## Step 1: Retrieve Deployment Access Credentials

Open the license dashboard, select the license you want to deploy, and reveal the deployment access credentials. Copy the deploy environment snippet. It contains the image source, metadata URL, and the access ID/secret used by the deploy scripts:

```dotenv
REGISTRY_HOST=<from dashboard>
REGISTRY_TOKEN_SERVICE_URL=<from dashboard>
REGISTRY_ACCESS_ID=<from dashboard>
REGISTRY_ACCESS_SECRET=<from dashboard>
RELEASE_METADATA_URL=<from dashboard>
```

Keep these values private. The deploy and update scripts use them to fetch the entitled release metadata and pull the images allowed by the license.
...
3. **Image access** - paste the access values from Step 1.
...
The script auto-generates `.env.secrets` (cryptographic keys, passwords, database URIs) if it does not exist yet, authenticates with the configured image source, pulls images, and starts all services.

The first successful image pull can bind the host's public IP to the license access grant, depending on your deployment policy.
...
Open the Admin frontend and navigate to **System > Licensing**. The admin backend creates the installation ID on first startup. Activate the license from the same host that pulled the images so the deployment binding and license activation match.
...
The updater fetches the entitled release manifest from the metadata endpoint using your deployment access credentials, validates image digests, and redeploys changed components. If the metadata endpoint is unreachable, the update aborts; there is no local fallback.
...
- [Provider Setup](./provider-setup) - connect Stripe, Mollie, GoCardless, Wise, PayPal, or wire transfer
