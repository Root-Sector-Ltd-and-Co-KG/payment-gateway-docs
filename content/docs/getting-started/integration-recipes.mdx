...
> [!NOTE]
> Screenshot placeholder: **Settings > API Keys** showing a least-privilege `checkout:create` key and the one-time secret reveal state.
...
- Set item `itemType` consistently (`virtual`, `physical`, `shipping`, or `discount`).
...
3. Register the webhook URL in Stripe or GoCardless.
4. Save the provider webhook signing secret on the provider record.
...
> [!NOTE]
> Screenshot placeholder: **Providers > Edit Provider** showing the provider ID and webhook secret fields.
...
> [!NOTE]
> Screenshot placeholder: **Transactions > Transaction Detail > IPN Information** showing delivery attempts, last status, and retry state.
...
sequenceDiagram
    ...
    App->>Admin: POST /v1/checkouts/{siteId}/create
